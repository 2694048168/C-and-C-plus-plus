...
private:
    ...
    {
        for (const std::string &commentPrefix : commentPrefixes_) eraseComment(commentPrefix, str);
    }
    ...
    std::vector<std::string>::const_iterator findCommentPrefix(const std::string &str, const std::size_t startpos) const
    {
        ...
        for (size_t i = 0; i < commentPrefixes_.size(); ++i)
        {
            ...
            bool match = true;
            for (size_t j = 0; j < prefix.size() && match; ++j) match = str[startpos + j] == prefix[j];
...
public:
    ...
    IniFileBase(const char fieldSep, const char comment)
        : fieldSep_(fieldSep)
        , commentPrefixes_(1, std::string(1, comment))
    {
    }
