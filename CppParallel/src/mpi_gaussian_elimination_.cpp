/**
 * @file mpi_gaussian_elimination_.cpp
...
int main(int argc, const char *argv[])
{
    ...
    {
        MPI_Scatter(matrix.get() + i * dim * num_tasks, dim, MPI_FLOAT, m_chunk.get() + i * dim, dim, MPI_FLOAT, 0,
                    MPI_COMM_WORLD);
    }
    ...
    for (int i = 0; i < n_rows; i++)
    {
        MPI_Gather(m_chunk.get() + i * dim, dim, MPI_FLOAT, matrix.get() + num_tasks * i * dim, dim, MPI_FLOAT, 0,
                   MPI_COMM_WORLD);
    }
