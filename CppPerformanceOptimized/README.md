...
- promise 和 future 无法被复制, 编程惯用法后使用 std::move(promise) 将其作为右值引用传递
...
- 高性能内存管理器, C++ 的默认内存管理器:
    ...
    - 必须能够高效地分配许多不同大小的对象;例如字符串
    - 必须既能够分配非常大的数据结构(I/O 缓冲区,含有数百万个整数值的数组),也能够分配非常小的数据结构(例如一个指针)
    - 为了使性能最大化,必须至少知道较大内存块的指针的对齐边界、缓存行和虚拟内存页
    - 运行时性能不能随着时间而降低
    - 必须能够高效地复用返回给它的内存
- 最先进的内存管理器可能会使用各种高速缓存和未使用的内存块池来提供性能
- 提供类专用内存管理器:如果一个类实现了 new() 运算符,那么当为该类申请内存时就不会调用全局 new() 运算符,而是调用这个 new() 运算符
- 编写高效地处理分配相同大小内存的请求的内存管理器是很容易的
- 自定义标准库分配器, C++ 标准库的容器类会使用大量的动态变量
- 标准库的容器类接收一个 Allocator 参数,它具有与类专用 new() 运算符相同的自定义内存管理器的能力
