/**
 * @File    : trainSVM.cpp
 * @Brief   : 基于机器学习方法，即支持向量机（Support Vector Machines， SVM）和基于梯度的直方图进行目标和行人的检测
 * @Author  : Wei Li
 * @Date    : 2021-08-11
*/
                        ...
                        int nBins)            // number of bins
{
    ...
    for (int i = 0; i < image.rows / cellSize.height; i++)
    {
        for (int j = 0; j < image.cols / cellSize.width; j++)
        {
            ...
            drawHOG(itDesc, nBins, hogImage(cv::Rect(j * cellSize.width, i * cellSize.height, cellSize.width, cellSize.height)), scale);
            itDesc += nBins;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    hogDesc.compute(cv::imread("./../images/stopSamples/stop08.png", cv::IMREAD_GRAYSCALE), desc);
    for (int i = 0; i < featureSize; i++)
        queries.ptr<float>(0)[i] = desc[i];
    hogDesc.compute(cv::imread("./../images/stopSamples/stop09.png", cv::IMREAD_GRAYSCALE), desc);
    for (int i = 0; i < featureSize; i++)
        queries.ptr<float>(1)[i] = desc[i];
    hogDesc.compute(cv::imread("./../images/stopSamples/neg08.png", cv::IMREAD_GRAYSCALE), desc);
    for (int i = 0; i < featureSize; i++)
        queries.ptr<float>(2)[i] = desc[i];
    hogDesc.compute(cv::imread("./../images/stopSamples/neg09.png", cv::IMREAD_GRAYSCALE), desc);
    for (int i = 0; i < featureSize; i++)
    ...
    // People detection
    cv::Mat myImage = imread("./../images/person.jpg", cv::IMREAD_GRAYSCALE);
