...
// The interface of frame processor
...
private:
    ...
    // The callback function to be called for the peocessing of each frame
    // 处理每一帧时都会调用的回调函数
    ...
    {
        if (extension.length()) // write images
        {
            std::stringstream ss;
            ss << outputFile << std::setfill('0') << std::setw(digits) << currentIndex++ << extension;
            cv::imwrite(ss.str(), frame);
        }
        else // write video file
        {
            writer.write(frame);
        }
    }
...
public:
    ...
    VideoProcessor() : callIt(false), delay(-1), fnumber(0),
                       stop(false), digits(0), frameToStop(-1), process(0), frameProcessor(0) {}
