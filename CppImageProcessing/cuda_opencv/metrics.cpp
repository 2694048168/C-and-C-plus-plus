/**
 * @brief Goal
 * Similarity measurement tutorial presented the PSNR and SSIM methods
 * for checking the similarity between the two images. And as you could see,
 * the execution process takes quite some time, especially in the case of
 * the SSIM. However, if the performance numbers of an OpenCV implementation
 * for the CPU do not satisfy you
 * and you happen to have an NVIDIA CUDA GPU device in your system
 * You may try to port or write your owm algorithm for the video card.
 *
 * This tutorial will give a good grasp on how to approach coding
 * by using the GPU module of OpenCV. As a prerequisite you should already
 * know how to handle the core, highgui and imgproc modules.
 *
 * So, our main goals are:
 * 1. What's different compared to the CPU?
 * 2. Create the GPU code for the PSNR and SSIM
 * 3. Optimize the code for maximal performance
 *
...
#include <opencv2/core/utils/logger.hpp> /* setLogLevel */
...
/* The PSNR returns a float number, that if the two inputs are
 similar between 30 and 50 (higher is better).

 The SSIM returns the MSSIM of the images. This is too a floating point
 number between zero and one (higher is better),
 however we have one for each channel.
-------------------------------------------------------------------- */
...
/* Optimized CUDA versions. Data allocations are very expensive on CUDA.
 Use a buffer to solve: allocate once reuse later.
 -------------------------------------------------- */
struct BufferPSNR
{
...
/**
 * @function Image metrics for PNSR and SSIM
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Time of PSNR CPU (averaged for "
              << TIMES << " runs): "
              << time << " milliseconds."
              << " With result of: "
              << result << std::endl;
    ...
    std::cout << "Time of PSNR CUDA (averaged for "
              << TIMES << " runs): "
              << time << " milliseconds."
              << " With result of: "
              << result << std::endl;
    ...
    std::cout << "Initial call CUDA optimized:              "
              << time << " milliseconds."
              << " With result of: "
              << result << std::endl;
    ...
    std::cout << "Time of PSNR CUDA OPTIMIZED ( / "
              << TIMES << " runs): "
              << time << " milliseconds."
    ...
    std::cout << "Time of MSSIM CPU (averaged for "
              << TIMES << " runs): "
              << time << " milliseconds."
              << " With result of B" << x.val[0]
    ...
    std::cout << "Time of MSSIM CUDA (averaged for "
              << TIMES << " runs): "
              << time << " milliseconds."
              << " With result of B" << x.val[0]
              << " G" << x.val[1] << " R" << x.val[2] << endl;
    ...
    std::cout << "Time of MSSIM CUDA Initial Call            "
              << time << " milliseconds."
              << " With result of B" << x.val[0]
              << " G" << x.val[1] << " R" << x.val[2] << endl;
    ...
    std::cout << "Time of MSSIM CUDA OPTIMIZED ( / " << TIMES << " runs): " << time << " milliseconds."
              << " With result of B" << x.val[0]
              << " G" << x.val[1] << " R" << x.val[2] << std::endl;
