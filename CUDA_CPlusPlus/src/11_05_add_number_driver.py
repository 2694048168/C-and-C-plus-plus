...
#
# @author: weili
# @filename: 11_05_add_number_driver.py
...
# @function: pycuda 使用 Python 利用 CDUA 进行两个数相加, 使用 driver 类简化程序。
...
add_num(drv.Out(host_result), drv.In(host_a), drv.In(host_b), block=(N,1,1), grid=(1,1))
...
for i in range(0, N):
  print(host_a[i],"+", host_b[i] , "=" , host_result[i])
