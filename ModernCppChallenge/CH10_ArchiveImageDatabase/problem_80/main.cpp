 ...
 * @brief Compressing and decompressing files to/from a ZIP archive
 * @version 0.1
 * @date 2024-01-27
 * 
 ...
 *  to a ZIP archive
 * 2. Decompress the contents of a ZIP archive to a user-specified destination directory
 * 
 */
...
------------------------------------------------------ */
void compress(const fs::path &source, const fs::path &archive, std::function<void(std::string_view)> reporter)
{
...
void decompress(const fs::path &destination, const fs::path &archive, std::function<void(std::string_view)> reporter)
{
