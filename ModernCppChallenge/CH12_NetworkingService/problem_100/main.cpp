 ...
 * @brief Detecting faces in a picture
 * @version 0.1
 * @date 2024-01-30
 * 
...
{
public:
    face_manager(std::string_view endpoint, std::string_view key)
        : endpoint(endpoint)
        , app_key(key)
    {
    }
...
private:
    face_detect_response parse_detect_response(const long status, std::stringstream &str)
    {
        json jdata;
        str >> jdata;
        ...
        try
        {
            ...
            else if (status >= 400)
            {
                ...
                std::cout << response.error.code << std::endl << response.error.message << std::endl;
            }
        }
        catch (const std::exception &ex)
        {
...
{
    face_manager manager("https://westeurope.api.cognitive.microsoft.com/face/v1.0", "...(your api key)...");
    ...
    for (const auto &face : results)
    {
        std::cout << "faceId: " << face.faceId << std::endl
                  ...
                  << "rect:   "
                  << "{" << face.rectangle.left << "," << face.rectangle.top << "," << face.rectangle.width << ","
                  << face.rectangle.height << "}" << std::endl
                  << std::endl;
    }
